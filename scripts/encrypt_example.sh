...
./rocketchat --rocketchat-url='https://chat.enarx.dev' --rocketchat-channel='general' --rocketchat-es-url="${ESURL}" --rocketchat-debug=1 --rocketchat-user="`cat ./secrets/user-ccc2.encrypted.secret`" --rocketchat-token="`cat ./secrets/token-ccc2.encrypted.secret`" --rocketchat-stream="${STREAM}" $* 2>&1 | tee run.log
