...
./rocketchat --rocketchat-url='https://chat.hyperledger.org' --rocketchat-channel='sawtooth' --rocketchat-es-url="${ESURL}" --rocketchat-debug=0 --rocketchat-user="`cat ./secrets/user.encrypted.secret`" --rocketchat-token="`cat ./secrets/token.encrypted.secret`" --rocketchat-stream="${STREAM}" $* 2>&1 | tee run.log
